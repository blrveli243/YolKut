...
class _GoalsScreenState extends ConsumerState<GoalsScreen>
  ...
  Future<void> _saveGoals() async {
    final data = {
      ...
      'targetWeight': _selectedGoal == 'Formu Koru'
          ? _currentWeight
          : _targetWeight,
    ...
    if (mounted && ref.read(profileProvider).hasError == false) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text(
            'Hedefler ve Plan Başarıyla Güncellendi! 🚀',
  ...
  Widget _buildGoalSelector() {
    return Row(
      ...
      children: [
        _goalCard('Kilo Ver', Icons.trending_down, AppColors.error),
        const SizedBox(width: 12),
        _goalCard('Formu Koru', Icons.monitor_weight_outlined, AppColors.info),
        const SizedBox(width: 12),
        _goalCard('Kas Yap', Icons.trending_up, AppColors.fat),
      ],
    );
  }
  ...
  Widget _buildSimulationCard(Map<String, dynamic> profile) {
    ...
    if (gender.toString().toLowerCase() == 'erkek') {
      bmr =
          88.362 + (13.397 * _currentWeight) + (4.799 * height) - (5.677 * age);
    } else {
      bmr =
          447.593 + (9.247 * _currentWeight) + (3.098 * height) - (4.330 * age);
    }

    final tdee = bmr * _activityLevel;
    final weightDiff =
        _currentWeight -
        (_selectedGoal == 'Formu Koru' ? _currentWeight : _targetWeight);
    ...
    Color themeColor = AppColors.info;
    String title = 'Mevcut Kiloyu Koru';
    String message =
        'Bu hızla sağlığınızı riske atmadan planınızı uygulayabilirsiniz.';
    ...
    if (_selectedGoal != 'Formu Koru') {
      if (isUnsafe && weightDiff > 0) {
        ...
        message =
            'Hedefinize bu sürede ulaşmak için güvenli sınır ($safeMin kcal) altına düşmeniz gerekiyor. Lütfen hedefinizi veya süreyi uzatın.';
        icon = Icons.warning_amber_rounded;
      } else if (weightDiff > 0) {
        themeColor = AppColors.primary;
        title = 'Sağlıklı Deficit (Açık)';
      } else {
        themeColor = AppColors.fat;
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  Text(
                    isUnsafe && weightDiff > 0
                        ? 'Riskli'
                        : '$targetCalories kcal',
                    style: TextStyle(
                      color: isUnsafe && weightDiff > 0
                          ? AppColors.error
                          : Theme.of(context).colorScheme.onSurface,
                      fontSize: 28,
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      Text(
                        'İlerleme ($completed / $total görev)',
                        style: TextStyle(
                          color: Theme.of(
                            context,
                          ).colorScheme.onSurface.withValues(alpha: 0.7),
  ...
  Widget _buildCustomTab() {
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [_buildCustomGoalsList(), const SizedBox(height: 80)],
      ),
    );
  }
