...
class UserProfileScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final userPosts = state.posts.where((p) => p.authorId == userId).toList();
    final isDark = Theme.of(context).brightness == Brightness.dark;

    return Scaffold(
      backgroundColor: Theme.of(context).scaffoldBackgroundColor,
      appBar: AppBar(
        title: Text(user.name, style: TextStyle(color: Theme.of(context).colorScheme.onSurface, fontWeight: FontWeight.bold)),
                ...
                children: [
                  Expanded(
                    child: ElevatedButton(
                      onPressed: () {
                        ref.read(communityProvider.notifier).toggleFriendStatus(userId);
                      },
                      style: ElevatedButton.styleFrom(
                        backgroundColor: _getFriendButtonColor(user.friendStatus, isDark),
                        foregroundColor: user.friendStatus == FriendStatus.none ? Colors.white : Theme.of(context).colorScheme.onSurface,
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        Navigator.push(context, MaterialPageRoute(
                          builder: (context) => ChatScreen(userId: userId, userName: user.name, avatar: user.avatar)
                        ));
                      },
                      style: ElevatedButton.styleFrom(
                      ...
                      child: const Text('Mesaj Gönder', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                    ),
                  ),
                ],
              ),
  ...
  String _getFriendButtonText(FriendStatus status) {
    switch (status) {
      case FriendStatus.none: return 'Arkadaş Ekle';
      case FriendStatus.pending: return 'İstek Gönderildi';
      case FriendStatus.accepted: return 'Arkadaşsınız';
    }
  }

  Color _getFriendButtonColor(FriendStatus status, bool isDark) {
    switch (status) {
      case FriendStatus.none: return const Color(0xFF0A84FF);
      case FriendStatus.pending: return isDark ? Colors.grey[800]! : Colors.grey[300]!;
      case FriendStatus.accepted: return isDark ? Colors.grey[800]! : Colors.grey[300]!;
    }
  }
  ...
  Widget _buildPostImage(String urlOrPath) {
    if (urlOrPath.startsWith('http')) {
      return Image.network(urlOrPath, width: double.infinity, fit: BoxFit.cover);
    } else {
      return Image.file(File(urlOrPath), width: double.infinity, fit: BoxFit.cover);
    }
  }
}
