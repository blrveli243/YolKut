  ...
  Future<Map<String, dynamic>> fetchHealthData() async {
    ...
    for (var p in activeEnergy) {
      if (p.value is NumericHealthValue) {
        totalActiveEnergy += (p.value as NumericHealthValue).numericValue
            .toDouble();
      }
    }
    ...
    for (int i = 0; i < 30; i++) {
      ...
      final dailySteps = await _health.getTotalStepsInInterval(
        start,
        i == 0 ? now : end,
      );
    ...
    for (var p in workoutData) {
      if (p.value is WorkoutHealthValue) {
        ...
        else if (activityType == 'TRADITIONAL_STRENGTH_TRAINING')
          activityType = 'Ağırlık Antrenmanı';
