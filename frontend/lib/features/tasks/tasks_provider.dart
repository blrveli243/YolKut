...
class TasksState {
  ...
  TasksState({required this.selectedDate, required this.tasks, required this.stats});
...
class TasksNotifier extends Notifier<TasksState> {
  ...
  Future<void> fetchTasksForDate(DateTime date) async {
    state = state.copyWith(selectedDate: date, tasks: const AsyncValue.loading(), stats: const AsyncValue.loading());
    try {
      ...
      if (hasWorkout) {
         final isCompleted = ref.read(workoutLogsProvider.notifier).isWorkoutCompletedToday(_formatDate(date));
         ...
         final dailyTotal = (stats['dailyTotal'] as num?)?.toInt() ?? tasks.length;
         final dailyCompleted = (stats['dailyCompleted'] as num?)?.toInt() ?? tasks.where((t) => t['isCompleted'] == true).length;
         final newDailyRate = dailyTotal > 0 ? (dailyCompleted / dailyTotal) * 100 : 0.0;
  ...
  Future<void> createTask(String title, {DateTime? scheduledTime, String? location, int reminderOffsetMinutes = 15}) async {
    try {
      final repo = ref.read(taskRepositoryProvider);
